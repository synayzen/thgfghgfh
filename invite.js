...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        if(channel){
       channel.send(` ${member} katıldı! **Davet Eden:** ${invite.inviter}  (**${total + bonus }** Davet! <a:Assassins_guvenilir:820061737868853298>) <:assassins_f:822359346376736788>`)
        }
    }).catch();
});
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
     	channel.send(`${member.user} ayrıldı! **Davet Eden:** ${user} (${Number(total) + Number(bonus)} Davet! <a:Assassins_tehlikeli:820061702870925342>) <:assassins_g:822359310935261195>`)
     }
});
